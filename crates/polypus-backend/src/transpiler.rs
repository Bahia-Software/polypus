...
//! [`crate::QuantumBackend`]: an execution backend *composes* the `Transpiler`
//! appropriate for its target and invokes it inside its own `run_circuits`, so
//! algorithm code never has to know about hardware-specific rewriting.
//!
...
/// keeping it usable at HPC scale. Each [`crate::QuantumBackend`] *composes* the
/// `Transpiler` appropriate for its target (the *strategy*), and passes per-run
/// [`TranspileOptions`] (the *tuning*); algorithm code stays completely unaware
/// of any hardware-specific rewriting.
///
/// # Extending
///
/// - **New strategy** (a different rewriting algorithm): create a type that
///   implements `Transpiler` and inject it by composition (e.g. the native
///   statevector backend's `with_transpiler`). No change to
///   [`QuantumBackend`](crate::QuantumBackend), the algorithms, or
///   [`BoundCircuit`](crate::BoundCircuit).
